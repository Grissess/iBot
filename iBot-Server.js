	...
	this.cmdHelp = function (server, invoker, target, cmdname) {
		if(typeof cmdname === 'undefined') {
			...
			for(var cmd in server.commands) {
				var cmds=server.commands[cmd].genNames();
				for(var i in cmds) {
					if(allcmds.indexOf(cmds[i])>=0) {
						allcmds.push(server.commands[cmd].module+'.'+cmds[i]);
					} else {
						allcmds.push(cmds[i]);
					}
				}
			...
			if(cmdname.indexOf('.')>=0) {
				...
				mod=split[0]
				cmdname=split.slice(1).join('.');
			}
			var cmds=[];
			for(var cmd in server.commands) {
				if(mod !== null && server.commands[cmd].module !== mod) {
					continue;
				}
				if(server.commands[cmd].genNames().indexOf(cmdname)>=0) {
					cmds.push(server.commands[cmd]);
				}
			}
			if(cmds.length>1) {
				var names=[];
				for(var i in cmds) {
					names.push(cmds[i].module+'.'+cmdname);
				}
				target.privmsg('Command conflict between '+names.join(',')+'. Please specify one from this list.');
			} else if(cmds.length==1) {
				target.privmsg(cmds[0].genHelp());
			} else {
